...
class Interpreter():
	...
	def __init__(self, dataInput):
		...
		dataInput = self.removeTabs(dataInput)
	...
	def getDataOutput(self):
		if (self.error):
			...
			saida = ""
			for i in teste1.dataOutput:
				saida += i + "\n"
			try: 
				servidorSocket = socket.socket()
				servidorSocket.connect((HOST, PORT))
				res = servidorSocket.send(saida.encode())
				return [True,"Arquivo compilado e executado na plaquinha de IP "+HOST+"."]
			except:
			
				print ("Sem conexão")
				# se a conexao funcionar manda mensagem de ok
				# senão mostra mensagem que compilou, mas não existe uma conexão ativa.
				return [True,self.dataOutput]
	...
	def cmdLoop(self):
		...
		if (line[0] == "comando_loop"):
			...
			if (self.dataInput[self.i] == "fim_comando_loop"):
				return True	
			else:
				return False
	...
	def cmdOutput(self, line, j, indentation):
		if (line[j] == "ligar"):
			...
			#print(idDevice)
			if (idDevice in self.devicesTable):
				if (self.devicesTable[idDevice][0] in ('pwm', 'digitsIN')):
...
arquivo = open("../casosDeTeste/semErros/entrada/exemplo1.faz")
string1 = arquivo.read()
teste1 = Interpreter(string1)
print(teste1.startInterpreter())
